...
def batch_predict_varying_T(latent_dim, covar_module0, covar_module1, likelihoods, prediction_x, test_x, mu, zt_list, id_covariate, eps):
    
    # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    device = prediction_x.device
...
def batch_predict(latent_dim, covar_module0, covar_module1, likelihoods, prediction_x, test_x, mu, zt_list, P, T, id_covariate, eps):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
def plot_training_info(net_train_loss=None, net_train_nll=None, net_train_KL_Z=None, net_train_mean_error=None,
                       tr_imputed_error=None,
                       net_test_mean_error=None, test_imputed_error=None, save_path=None, validation_loss=None):
    try:
        fig, ax1 = plt.subplots()
        color = 'tab:red'
        ax1.plot(net_train_loss, color=color, label='Net Loss')
        ax1.tick_params(axis='y', labelcolor=color)
        ax1.legend(loc=1)

        plt.savefig(save_path + '/training_net_loss')
    except:
        pass
    ...
    try:
        fig, ax1 = plt.subplots()
        color = 'tab:red'
        ax1.plot(-1 * np.array(net_train_nll), color=color, label='Training Recon LogLik per Variable')
        ax1.tick_params(axis='y', labelcolor=color)
        ax1.legend(loc=1)
        ...
        color = 'tab:blue'
        ax2.plot(net_train_KL_Z, color='tab:blue', label='Training KL z')
        ax2.tick_params(axis='y', labelcolor=color)
        ax2.legend(loc=3)
        fig.tight_layout()  # otherwise the right y-label is slightly clipped,
        plt.savefig(save_path + '/training_kl_ll')
    except:
        pass
    ...
    try:
        ...
        color = 'tab:blue'
        ax2.plot(net_train_mean_error, color='tab:green', label='Training mean or VAE error')
        try:
            ax2.plot(np.repeat(torch.mean(tr_imputed_error, 0).cpu().numpy(), len(net_train_mean_error)),
                     color='b', label='Training imputation error or VAE error')
        except:
            pass
        ax2.tick_params(axis='y', labelcolor=color)
        ax2.legend(loc=0)
        fig.tight_layout()  # otherwise the right y-label is slightly clipped
        plt.savefig(save_path + '/training_VAE_error')
    except:
        pass
    ...
    try:
        fig, ax2 = plt.subplots()
        color = 'tab:blue'
        ax2.plot(net_test_mean_error, color='tab:green', label='Test mean/GP error')
        try:
            ax2.plot(np.repeat(torch.mean(test_imputed_error, 0).cpu().numpy(), len(net_test_mean_error)),
                     color='b', label='Test imputation/GP error')
        except:
            pass
        ax2.tick_params(axis='y', labelcolor=color)
        ax2.legend(loc=0)
        fig.tight_layout()  # otherwise the right y-label is slightly clipped
        plt.savefig(save_path + '/test_GP_error')
    except:
        pass
    try:
        fig, ax1 = plt.subplots()
        color = 'tab:red'
        ax1.plot(validation_loss, color=color, label='Validation Loss')
        ax1.tick_params(axis='y', labelcolor=color)
        ax1.legend(loc=1)

        fig.tight_layout()  # otherwise the right y-label is slightly clipped,
        plt.savefig(save_path + '/validation_net_loss')
    except:
        pass
